...
fg_api.prototype.adsModule = function() {
	...
	module.prototype.init = function() {
		...
		if (M.provider == 'adsense' || M.provider == 'adx') {
			M.provider = 'dfp';
		}

		// Halt the loading of the game until showAd() callback is fired.
		self.game.setWaiting(self.config.ads.show_initial);

		var sdkLoadedCallback = function() {
			if (self.config.ads.show_initial) {
				if (self.config.ads.show_video || self.config.gameParams.orientation == "landscape") {
					M.playBtn = self.createElement("div", {
						"class": "fg-click2play"
					});
					M.playBtn.innerHTML =
						'<div class="btn-play" title="Click to Play!"></div>';
					self.rootElement.appendChild(M.playBtn);

					function startVideoCallback() {
						faZepto(M.playBtn).remove();
					...
					if (detection.is.pc) {
						...
						self.spinner.hide();
						M.playBtn.style.visibility = 'visible';
						M.playBtn.style.opacity = 1;
					}
				} else {
					M.showAd(function() {
						self.game.setWaiting(false);
	...
	module.prototype.showAd = function(callback, force) {
		...
		switch (M.provider) {
			...
			case 'ad4game':
				return M.showAd4GameInterstitial(callback, force);
			case 'leadbolt':
				return M.showLeadbolt(callback, force);
			case 'gtburst':
				return M.showGTB(callback, force);
			case 'housead':
	...
	module.prototype.showLeadbolt = function(callback, force) {
		if (M.adcount > 1) {
			return M.showLeadboltInterstitial(callback, force);
		}
	...
	module.prototype.showIMA = function(callback, force) {
		...
		M.adContainerElement.style.width = Math.min(window.innerWidth - 20, 728) + "px";
		...
		// M.videoElement = document.createElement('video');
		// M.videoElement.setAttribute('width', 'auto');
		// M.videoElement.setAttribute('height', 'auto');
		// M.adContainerElement.appendChild(M.videoElement);

		// if (typeof window.videojs != "undefined") {
		// 	window.videojs(M.videoElement, {}, function() {
		// 		self.log("videojs ready");
		// 	});
		// }

		try {
			// M.adDisplayContainer = new google.ima.AdDisplayContainer(M.adContainerElement, M.videoElement);
		...
		module.prototype.onAdsManagerLoaded = function(adsManagerLoadedEvent) {
			...
			M.adsManager.init(parseInt(M.adContainerElement.style.width), parseInt(M.adContainerElement
				.style.height), google.ima.ViewMode.NORMAL);
			...
			switch (adEvent.type) {
				case google.ima.AdEvent.Type.LOADED:
					...
					if (ad.isLinear() && M.adsManager.getRemainingTime() > 0) {
						self.modal.setDimensions(parseInt(M.adContainerElement.style.width),
												 parseInt(M.adContainerElement.style.height));
				...
				case google.ima.AdEvent.Type.STARTED:
					// update timer when impression was counted for video
					if (ad.isLinear()) {
						if (M.adsManager.getRemainingTime() > 0) {
							self.modal.setCloseBtnTimer(parseInt(M.adsManager.getRemainingTime()), {
								"autoclose": false
							});
						}
					}
				...
				case google.ima.AdEvent.Type.FIRST_QUARTILE:
					// update timer when video is playing to account for delay during start
					if (M.adsManager.getRemainingTime() > 0 && !M.adsManager.getAdSkippableState()) {
						self.modal.setCloseBtnTimer(parseInt(M.adsManager.getRemainingTime()), {
							"autoclose": false
						});
					}
				...
				case google.ima.AdEvent.Type.SKIPPABLE_STATE_CHANGED:
					// update close button, the ad is now skippable
					if (M.adsManager.getAdSkippableState()) {
						self.modal.updateCloseBtn();
					}
					break;
		...
		M.dfp_custom_params = {
			...
			"v": self.config.ads.show_video ? "1" : "0",
		...
		if (self.debug && self.config.ads.show_video) {
			adsRequest.adTagUrl = 'https://pubads.g.doubleclick.net/gampad/ads?sz=640x360&iu=/6062/iab_vast_samples/skippable&ciu_szs=300x250,728x90&impl=s&gdfp_req=1&env=vp&output=xml_vast2&unviewed_position_start=1&url=[referrer_url]&correlator=[timestamp]';
		}
		...
		self.config.ads.show_video = false;
	};
	...
	module.prototype.showAd4GameInterstitial = function(callback, force) {
		var opts = {
			mode: "",
		...
		self.log("show ad4game interstitial");
		...
		// show banner
		ad4game_html5_sdk.setBrand(self.getMoreGamesButtonImage(), self.translate(
			"more_games_url"), "Find more games on");
	...
	module.prototype.showLeadboltInterstitial = function(callback, force) {
		var opts = {
			mode: "ad",
			min_s_between: (+self.config.ads.min_s_between > 0) ? +self.config.ads.min_s_between : 30000,
			iframeAllowed: true,
		...
		self.log("show leadbolt interstitial");
		...
		// show iframe
		var url = '/sda/interstitial/' + window.famobi_gameID + '/' + window.famobi.config
			.aid;
	...
	module.prototype.showGTB = function(callback, force) {
		var opts = {
			mode: "ad",
			min_s_between: (+self.config.ads.min_s_between > 0) ? +self.config.ads.min_s_between : 30000,
			iframeAllowed: true,
		...
		self.log("show gtburst interstitial");
		...
		// show iframe
		var w = 300, h = 250, url = '/sda/interstitial/' + window.famobi_gameID + '/' + window.famobi.config.aid;
	...
	module.prototype.showAppWall = function(callback, force) {
		...
		// show iframe
		var url = '/sda/appwall/' + window.famobi_gameID + '/' + window.famobi.config
			.aid;
		self.iframe.show(url, opts);
		self.modal.setHeader(self.translate("api.ad_modal_header2"));
		self.modal.setCloseCallback(opts.returnCallback);
	...
	module.prototype.showBanner = function(callback, force) {
		self.log("show banner");
	...
	module.prototype.delayNextAdModal = function(isSetter) {
		...
		if (isSetter) {
			var min_s_between = (+self.config.ads.min_s_between > 0) ? +self.config.ads
				.min_s_between : 30000;
...
fg_api.prototype.screenshotModule = function() {
	...
	function handleScreenshotSave(data){
		...
		M.savingInterval = setInterval(function() {
			image.src = data.url+"?foc="+Math.random()*1E16;
...
fg_api.prototype.orientationModule = function() {
	...
	orientationPrototype.init = function(){
		// otherwise show rotation screen
...
fg_api.prototype.gametranslationModule = function() {
	...
	gameTranslationPrototype.getGameTranslations = function() {
		...
		i18n.current = faZepto.extend(
			i18n["default"],
			i18n[lang] ? i18n[lang] : [],
			i18n[self.config.aid+'.'+lang] ? i18n[self.config.aid+'.'+lang] : []
		);
...
//		self.ads 			= require("ads");
	...
	moreGamesImg.onerror = function() {
		if (self.config.game_i18n.current) {
			self.config.game_i18n.current.more_games_image = "./gameapi/branding/default/More_Games600x253_transparent.png";
		}
//		advance();
	};
