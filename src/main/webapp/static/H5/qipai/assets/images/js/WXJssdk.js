...
WXJssdkInit = function(options) {
    ...
    if (!options) options = {};
    var jssdk = location.protocol+'//res.wx.qq.com/open/js/jweixin-1.0.0.js'; //微信的sdk
    var jssdkGetSignature = location.protocol+"//apps.game.qq.com/ams/wx/jo/getSign.php" //获取jssdk签名接口包含参数sServiceType=feiji和sUrl
	...
	*/
    this.init = function(fn, appId, jsApiList, ifDebug) {
		...
		if(typeof window.WxjssdkWx=='object'){
			console.log('wxjssdk object is ok,but repeat init, maybe cause problem.');
			fn(WxjssdkWx);
			return;
		}
		if(window.WxjssdkWx==1){
			console.log('wxjssdk repeat init, maybe cause problem.');
			return;
		}
		window.WxjssdkWx=1;
...
WXJssdk = new WXJssdkInit();/*  |xGv00|7f528ea47df65026b6154ecdd9290348 */