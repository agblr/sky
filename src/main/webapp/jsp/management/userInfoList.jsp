...
<c:set var="preUrl" value="userInfoList
	...
	<div class="mini-title">当前位置：用户管理</div>
	</header>
	<article class="container-fluid">
		<form class="form-inline search-form">
			<div class="form-group">
				<label>角色</label>
				<select id="role" name="groupName" class="form-control"></select>
			</div>
			<div class="form-group">
				<label>账号状态</label>
				<select name="status" class="form-control">
					<option value="">全部</option>
					<c:set var="enumPersonStatus" value="<%= EnumPersonStatus.values() %>"/>
					<c:forEach var="enumPersonStatu" items="${ enumPersonStatus }">
					<option value="${ enumPersonStatu.id }" <c:if test="${ enumPersonStatu.id == status }">selected = "selected"</c:if>>${ enumPersonStatu.desc }</option>
					</c:forEach>
				</select>
			</div>
			<div class="form-group">
			...
			<div class="form-group">
				<a href="#" class="btn btn-success ml10" id="add-member-btn">创建用户</a>
			</div>
		</form>
