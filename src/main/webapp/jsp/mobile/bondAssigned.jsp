...
<body>
	<div class="login search_t">
		<h2>
			...
			<img src="<%=request.getContextPath() %>/mobile/images/poi_lt.jpg" /></a>已转让债权
		</h2>
	</div>
	<c:forEach items="${page.voList}" var="data" varStatus="i">
		<div class="index_c m_search"><h3><a >${data.title}</a></h3>
			<div class="debt_ass_l">
			  <p><em>转让价格:</em>&nbsp;${data.tradePrice}元</p>
			  <p><em>手&nbsp;&nbsp;续&nbsp;&nbsp;费:</em>&nbsp;${data.tradeCommissionFee}元</p>
			  <p><em>交易时间:</em>&nbsp;<fmt:formatDate value="${data.tradeTime}" pattern="yyyy-MM-dd HH:mm"/></p>
			</div>
			<div class="debt_ass_l debt_ass_r tc"><p class="blank10"></p><p class="blank10"></p><p><b>${data.annualInterestRate}%</b></p></div>
		 	<div class="clear"></div>
	 	</div>
	</c:forEach>
	 ...
	 jQuery(document).ready(function($) {
		...
		function addText(){
			...
			currentPage++;
			 $.ajax({  
				   type:"POST", //请求方式  
			       url:"${base}/listMobile3.action", //请求路径   
			       ...
			       success:function(data){
			    	   ...
			    	   $(data).each(function(index, element) {
			    	   	str = '<div class="index_c m_search"><h3><a >'+element.title+'</a></h3>'+
								'<div class="debt_ass_l">'+
								  '<p><em>转让价格:</em>&nbsp;'+Number(element.tradePrice).toFixed(2)+'元</p>'+
								  '<p><em>手&nbsp;&nbsp;续&nbsp;&nbsp;费:</em>&nbsp;'+Number(element.tradeCommissionFee).toFixed(2)+'元</p>'+
								  '<p><em>交易时间:</em>&nbsp;'+element.formatTradeTime+'</p>'+
								'</div>'+
								'<div class="debt_ass_l debt_ass_r tc"><p class="blank10"></p><p class="blank10"></p><p><b>'+Number(element.annualInterestRate).toFixed(2)+'%</b></p></div>'+
							 	'<div class="clear"></div>'+
						 	  '</div>';
