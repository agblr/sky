...
<body>
	...
	<div class="d_invest_c">
		  <dl>
		   <dd>基本资料<p class="on"></p></dd><dd>借款详情<p></p></dd><dd>借款材料<p></p></dd><dd>投资记录<p></p></dd><p class="clear"></p>
		  </dl>
		  <ul style="display:block">
		   <li>
		     <c:if test="${ not empty loanPic}">
              <img height="26" width="26" src="${base }/noLoginCreditMaterial.action?urlpath=${loanPic.path}/prototype/${loanPic.picName}">
            </c:if>
		   ${bizLoaner.bizName}</li>
		   <c:if test="${not empty bizLoaner.taxCode}">
		      <li>税务登记证 <img class="exist" src="${base}/mobile/images/invest.jpg"></li>
		   </c:if>
		   <c:if test="${not empty bizLoaner.instructions}">
		      <li>营业执照 <img class="exist" src="${base}/mobile/images/invest.jpg"></li>
		   </c:if>
		    <c:if test="${not empty bizLoaner.organizationCode}">
		      <li>组织机构代码证 <img class="exist" src="${base}/mobile/images/invest.jpg"></li>
		   </c:if>
		     <c:if test="${not empty bizLoaner.classification}">
		      <li>银行开户许可证 <img class="exist" src="${base}/mobile/images/invest.jpg"></li>
		   </c:if>
		  </ul>
		<ul>
			<li>1、投资期限：<c:choose>
