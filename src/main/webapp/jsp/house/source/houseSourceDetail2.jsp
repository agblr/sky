...
<%@ page import="com.xmbl.ops.enumeration.EnumSubject" %>
...
<html lang="zh-CN">
	...
	<body>
		<header class="ui-page-header">
			<div class="mini-title">房屋详细</div>
		</header>
		<ul id="special-nav-tabs" role="tablist" class="nav nav-tabs">
			<li role="presentation">
				<a id="special-outline-tab" href="#special-outline" role="tab" data-toggle="tab">概述</a>
			</li>
			<li role="presentation" class="active">
				<a id="special-edit-tab" href="#special-edit" role="tab" data-toggle="tab">编辑</a>
			</li>
		</ul>
		<article id="special-tab-content" class="tab-content">
			<section id="special-outline" role="tabpanel" class="tab-pane fade">
				<div class="panel panel-default">
					<div class="panel-heading">基本信息</div>
					<div class="panel-body form-horizontal">
						<div class="form-group">
							<label class="col-sm-1 control-label">专题名称：</label>
							<div class="col-sm-5 pl0 ui-form-display">${ special.title }</div>
							<label class="col-sm-1 control-label">专题ID：</label>
							<div class="col-sm-5 pl0 ui-form-display">${ special.id }</div>
						</div>
						<div class="form-group">
						...
						<div class="form-group">
							...
							<div class="col-sm-5 pl0 ui-form-display">
							${ func:formatDate(special.createTime)}</div>
							<label class="col-sm-1 control-label">创建人：</label>
							<div class="col-sm-5 pl0 ui-form-display">${ special.operator }</div>
						</div>
					</div>
				</div>
				<div class="panel panel-default">
					<div class="panel-heading">数据统计</div>
					<div id="special-statistic-container" class="panel-body form-horizontal">
						<div class="form-group">
							<label class="col-sm-1 control-label">累计点赞(次)：</label>
							<div class="col-sm-5 pl0 ui-form-display">${ specialStatistics.praiseCount }</div>
							<label class="col-sm-1 control-label">后台点赞(次)：</label>
							<div class="col-sm-5 pl0">
								<input type="text" maxlength="15" class="special-like form-control" value="${ specialStatistics.praiseCountSys }">
							</div>
						</div>
						<div class="form-group">
							<label class="col-sm-1 control-label">累计转发(次)：</label>
							<div class="col-sm-5 pl0 ui-form-display">${ specialStatistics.shareCount }</div>
							<label class="col-sm-1 control-label">后台转发(次)：</label>
							<div class="col-sm-5 pl0">
								<input type="text" maxlength="15" class="special-reply form-control" value="${ specialStatistics.shareCountSys }">
							</div>
						</div>
						<div class="form-group">
							<label class="col-sm-1 control-label">累计评论(次)：</label>
							<div class="col-sm-5 pl0 ui-form-display">${ specialStatistics.replyCount }</div>
						</div>
						<button class="btn btn-primary pull-right btn-save">保存</button>
					</div>
				</div>
			</section>
			<section id="special-edit" role="tabpanel" class="tab-pane fade in active">
				<div class="ui-flex-between">
					<article class="special-edit-container flex-grow">
						<section id="special-base-info" class="special-base-info panel panel-default">
							<div class="panel-heading">基本信息</div>
							<div class="panel-body">
								<div class="form-group">
									<label>标题</label>
									<input type="text" placeholder="请填写标题，必填，最多15个字" value="${ special.title }" class="form-control special-main-title">
								</div>
								<div class="form-group">
									<label>背景</label>
									<div>
										<img alt="点击修改图片" title="点击修改图片" class="special-background" <c:if test="${ special.titleUrl != null }">src="${ special.titleUrl }"</c:if>>
									</div>
								</div>
								<div class="form-group">
									<label>介绍</label>
									<textarea rows="3" class="form-control special-description" placeholder="请输入介绍，最长90个字">${ special.content }</textarea>
								</div>
								<c:set var="enumSubjects" value="<%=EnumSubject.values()%>" />
								<c:if test="${ special.type != 1 }">
								<div class="knowledge-selection-container">
									<div class="form-group">
										<label>年级</label>
										<div>
											<label class="radio-inline"><input type="radio" name="special-grade" value="${ enumGrade.id}" <c:if test="${ isSelected == 1 }">checked</c:if>>${ enumGrade.chineseName }</label>
										</div>
									</div>
									<div id="knowledge-container">
										<div class="form-group has-feedback">
											<label>知识点</label>
											<input type="text" placeholder="最多添加3个知识点" class="form-control search-key"><span class="glyphicon glyphicon-search form-control-feedback search-btn"></span>
											<div class="knowledge-list">
												<c:forEach var="specialKnowledge" items="${ specialKnowledgeList }">
												<span class="label label-default knowledge-tag" data-id="${ specialKnowledge.knowledgeId }">${ specialKnowledge.knowledgeName }<span class="glyphicon glyphicon-remove knowledge-remove"></span></span>
												</c:forEach>
											</div>
										</div>
										<select class="form-control knowledge-select mb20">
											<option value="">请选择知识点</option>
										</select>
									</div>
								</div>
								<div style="display:none" class="other-selection-container">
									<div class="form-group">
										<label>年级</label>
									</div>
									<div class="form-group">
										...
										<div>
											...
											<label class="checkbox-inline"><input type="checkbox" name="special-subjects" value="${ enumSubject.id }">${ enumSubject.chineseName }</label>
											</c:forEach>
										</div>
									</div>
								</div>
								...
								<c:if test="${ special.type == 1 }">
								<div class="knowledge-selection-container" style="display:none">
									<div class="form-group">
										<label>年级</label>
									</div>
									<div class="form-group">
										...
										<div>
											<c:forEach var="enumSubject" items="${ enumSubjects }">
											<label class="radio-inline"><input type="radio" name="special-subject" value="${ enumSubject.id }">${ enumSubject.chineseName }</label>
											</c:forEach>
										</div>
									</div>
									<div id="knowledge-container">
								...
								<div class="other-selection-container">
									<div class="form-group">
										<label>学科</label>
										<div>
											<c:forEach var="enumSubject" items="${ enumSubjects }">
											...
											<c:forEach var="subject" items="${ subjectList }">
												<c:if test="${subject == enumSubject.id }">
													<c:set var="isSelected" value="1"/>
												</c:if>
											</c:forEach>
											<label class="checkbox-inline"><input type="checkbox" name="special-subjects" value="${ enumSubject.id }"<c:if test="${ isSelected == 1 }">checked</c:if>>${ enumSubject.chineseName }</label>
											</c:forEach>
										</div>
									</div>
								</div>
								</c:if>
								<button class="btn btn-primary btn-save-special pull-right" <c:if test="${ special.status == 2 }">disabled</c:if>>保存</button>
							</div>
						</section>
							<div class="panel-body">
								<ul id="special-module-tabs" role="tablist" class="nav nav-tabs">
									...
									<li role="presentation" <c:if test="${status.first}">class="active"</c:if>>
										<a href="#special-module${status.count}" role="tab" data-toggle="tab">
											<span>模块${status.count}</span>
											<c:if test="${ special.status != 2}">
											<span class="glyphicon glyphicon-remove remove-module"></span>
											</c:if>
										</a>
									</li>
									</c:forEach>
									<li class="add-module" role="presentation">
										<a href="#">
