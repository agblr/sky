...
<body>
<c:set var="preUrl" value="taobaoBaseHouseList
							?id=${ id }&
							housename=${ housename }&
	...
	<div class="mini-title">当前位置：淘宝池房源管理</div>
	</header>
			...
			<tbody id="user-list">
				<c:forEach var="baseHouseInfo" items="${ baseHouseList }">
					<tr data-user-id="${ baseHouseInfo.id }" >
					    <td><a href="<%=basePath%>base/getTaobaoHousereSources?id=${ baseHouseInfo.id }" >${ baseHouseInfo.id }</a></td>
