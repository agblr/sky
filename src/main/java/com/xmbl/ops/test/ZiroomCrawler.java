...
//            System.out.println("-----------第3步：" +"------doc----"+doc.toString());
            final Elements contents = doc.select("div .key-list");
//            System.out.println("-----------第4步：" + contents.toString() );
            System.out.println("-----------第4步：" );
            final Elements contents1 = doc.select("div .item-mod");
            
            System.out.println("-----------第5步：" + contents1.toString() );
            
//            System.exit(0);
           
            for (final Element c : contents1) {
            	System.out.println("-----------第6步：" );
           	 System.out.println("元素1---：" + c.toString());
           
            	if(contents1.get(10) == c){
            		 System.out.println("-----------第7步：" );
                	 System.out.println("元素1---：" + c.toString());
                	 final String soj = c.select(".pic").first().attr("soj");
                      System.out.println("soj：" + soj);
                      final String href = c.select(".pic").first().attr("href");
                      System.out.println("href：" + href);
                      final String img = c.select(".pic").val();
                      System.out.println("img：" + img);
                      final String htm = c.select(".pic").html();
                      System.out.println("htm：" + htm);
                      final Document doc1 = Jsoup.parse(htm);
                      final Elements contents22 = doc1.getElementsByTag("img");
                      System.out.println("contents22：" + contents22);
                      final String contents22src = doc1.select("img").first().attr("src");
                      System.out.println("contents22src：" + contents22src);
                      // 图片
//                    final String img = c.select(".img").first().attr("src");
